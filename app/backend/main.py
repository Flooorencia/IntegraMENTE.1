...
from motor_ia import (
    ...
    generar_meditacion, clasificar_estado_animo, generar_explicacion_profunda,
)
...
async def ejercicio(
    ...
    if "mantra" in resultado:
        respuesta["mantra"] = resultado["mantra"]
        respuesta["mantra_explicacion"] = resultado.get("explicacion", "")
        respuesta["audio_mantra_base64"] = await texto_a_audio_base64(resultado["mantra"], voz_premium=True)

    respuesta["pregunta_seguimiento"] = resultado.get("pregunta_seguimiento", "¿Cómo te quedaste después de esto? ¿Qué notás ahora?")
    respuesta["audio_pregunta_seguimiento_base64"] = await texto_a_audio_base64(respuesta["pregunta_seguimiento"])
...
# -----------------------------------------------------------------
# ENDPOINT: explicación profunda — después del intercambio corto
# inicial, antes de elegir dominio.
# -----------------------------------------------------------------
@app.post("/api/explicacion")
async def explicacion(
    ...
    resultado = await asyncio.to_thread(
        generar_explicacion_profunda, relato_original, historial
    )
    audio_b64 = await texto_a_audio_base64(resultado["texto"])
    return JSONResponse({
        "texto": resultado["texto"],
...
async def meditacion(
    ...
    # Voz premium (ElevenLabs) para la meditación — es el único lugar de la
    # app donde se gasta la cuota gratuita, así rinde para todo el mes.
    audio_intro = await texto_a_audio_base64(resultado.get("intro", ""), voz_premium=True)
    audio_pregunta = await texto_a_audio_base64(resultado.get("pregunta_cierre", ""), voz_premium=True)

    audios_pasos = []
    for paso in resultado.get("pasos", []):
        audio = await texto_a_audio_base64(paso["texto"], voz_premium=True)
