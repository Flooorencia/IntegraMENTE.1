...
from motor_ia import (
    generar_diagnostico, generar_ejercicio, generar_ejercicio_guiado, continuar_conversacion,
...
async def ejercicio(
    ...
    # Voz premium (ElevenLabs) en la consigna, igual que en el resto de
    # Ejercicio del momento; fundamento y pregunta de seguimiento usan
    # voz normal por ser parte del cierre (mismo criterio que
    # /api/ejercicio-guiado/audio-cierre).
...
# -----------------------------------------------------------------
# ENDPOINT: Ejercicio del momento — guiado por pasos (misma estructura
# que la meditación: círculo, puntos, voz premium, música de fondo).
# Mismo criterio lazy: solo la intro trae audio ya generado; cada paso
# se pide bajo demanda a /api/meditacion/audio-paso (genérico, no le
# importa si el texto es de un ejercicio o una meditación). La única
# parte que usa voz normal es el cierre, ver /audio-cierre más abajo.
# -----------------------------------------------------------------
@app.post("/api/ejercicio-guiado")
async def ejercicio_guiado(
    dominio: str = Form(...),
    relato_texto: str = Form(...),
    variante_idx: int = Form(default=0),
):
    resultado = await asyncio.to_thread(
        generar_ejercicio_guiado, dominio, relato_texto, variante_idx
    )

    # Voz premium (ElevenLabs): la intro es parte del "momento especial" de
    # la herramienta, igual que en Meditación guiada. La única parte de
    # Ejercicio del momento que usa voz normal es el cierre — ver
    # /api/ejercicio-guiado/audio-cierre más abajo, que arranca la
    # pantalla "Sigamos charlando".
    ...
    return JSONResponse({
        ...
        "pregunta_cierre": resultado.get("pregunta_cierre", "¿Cómo te quedaste con esto?"),
        "audio_intro_base64": audio_intro,
    })


# -----------------------------------------------------------------
# ENDPOINT: audio de cierre del Ejercicio del momento (fundamento +
# pregunta de seguimiento), bajo demanda.
# FIX BUG REAL: antes estos dos textos nunca tenían audio generado en
# ningún lado — se mostraban en la charla posterior pero se quedaban
# mudos siempre, sin que el silenciador tuviera nada que ver. Se pide
# recién acá, cuando la persona YA terminó el ejercicio y está por
# entrar a la charla — no antes, para no demorar el arranque del
# ejercicio generando audio que a veces ni se llega a necesitar (si
# saltea antes de terminar, igual hace falta el fundamento, así que se
# genera en ese momento igual).
# Se devuelven los dos audios SEPARADOS (no concatenados en uno solo) a
# propósito: así el frontend puede reproducir el fundamento, esperar una
# pausa real, y recién después la pregunta — como una conversación con
# tiempo entre una frase y la otra, no un audio único corrido.
# -----------------------------------------------------------------
@app.post("/api/ejercicio-guiado/audio-cierre")
async def ejercicio_guiado_audio_cierre(
    fundamento: str = Form(...),
    pregunta_cierre: str = Form(...),
):
    # Voz normal (Edge-TTS): a diferencia de la intro y los pasos (que sí
    # usan ElevenLabs), este cierre entra a la pantalla "Sigamos
    # charlando" con voz normal — ahí es donde no debía sonar ElevenLabs.
    audio_fundamento = await texto_a_audio_base64(fundamento)
    audio_pregunta = await texto_a_audio_base64(pregunta_cierre)
    return JSONResponse({
        "audio_fundamento_base64": audio_fundamento,
        "audio_pregunta_seguimiento_base64": audio_pregunta,
    })
