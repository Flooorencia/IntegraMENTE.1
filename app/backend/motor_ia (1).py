...
# GEMINI_API_KEY_2, _3 y _4 son opcionales: si están cargadas, la app pasa
# a la siguiente sola cuando la anterior se queda sin cuota diaria (plan
# gratis: 20/día por modelo, por proyecto de Google Cloud).
API_KEY = os.environ.get("GEMINI_API_KEY")
API_KEY_2 = os.environ.get("GEMINI_API_KEY_2")
API_KEY_3 = os.environ.get("GEMINI_API_KEY_3")
API_KEY_4 = os.environ.get("GEMINI_API_KEY_4")
...
client_cuaternario = genai.Client(api_key=API_KEY_4) if API_KEY_4 else None
# Orden de intento: principal → secundaria → terciaria → cuaternaria. Se
# recorre la lista completa antes de rendirse, así sumar una clave más
# (una quinta, una sexta) no requiere tocar la lógica de _generar de nuevo
# — solo agregarla acá adentro de la tupla.
_CLIENTES = [c for c in (client, client_secundario, client_terciario, client_cuaternario) if c]
# FIX CUOTA (2da vuelta): gemini-2.5-flash devolvía 404 "no longer
# available to new users" — Google le cerró el acceso a cualquier clave
# de API creada después de cierta fecha, aunque el dashboard de límites
# lo siga mostrando como si tuviera cuota disponible (por eso no se veía
# venir hasta que rompió en producción). Gemini 2.0 Flash y Flash-Lite
# ya están dados de baja del todo desde el 1/06/2026.
# gemini-3.5-flash-lite es la versión liviana/económica de la familia
# actual (Gemini 3.5), con acceso abierto a cualquier cuenta y bastante
# menos demanda que gemini-3.6-flash (que es el más nuevo y el que más
# rápido agota su cuota gratis por ser el más usado del momento).
# FIX CALIDAD + CUOTA: se prueba primero con el modelo de mejor calidad
# (gemini-3.6-flash) en cada una de las claves disponibles — es el que
# mejor entiende matices y personaliza, así que es el que se usa siempre
# que haya cuota libre en alguna clave. Solo si TODAS las claves se
# quedaron sin cuota en 3.6 Flash, se pasa a gemini-3.5-flash-lite (más
# liviano, con su propia cuota separada) como red de contención — mejor
# una respuesta algo menos matizada que ninguna respuesta.
MODELO_PRINCIPAL = "gemini-3.6-flash"
MODELO_RESPALDO = "gemini-3.5-flash-lite"


def _generar(contents, json_mode: bool = False):
    """Genera contenido probando primero MODELO_PRINCIPAL en cada una de
    las claves disponibles (principal → 2da → 3ra → 4ta); si NINGUNA
    clave tiene cuota para ese modelo, se repite la vuelta completa con
    MODELO_RESPALDO. `contents` puede ser un prompt de texto o una lista
    de partes (como en la transcripción de audio).

    FIX PERSONALIZACIÓN: json_mode=True fuerza a Gemini a devolver JSON
    válido de verdad (response_mime_type), en vez de confiar solo en la
    instrucción de texto "respondé en JSON estricto". Antes, cualquier
    desvío de formato en la respuesta del modelo (una palabra de más,
    una coma de menos) rompía el parseo y la app caía silenciosamente en
    el texto de resguardo fijo — que es genérico porque no es la IA
    hablando, es el texto de emergencia. Con esto, esa caída debería ser
    rarísima — el resguardo queda solo para una caída real de todas las
    claves en los dos modelos, no para un tropiezo de formato."""
    if not _CLIENTES:
        raise RuntimeError("Sin clientes de Gemini disponibles")
    config = {"response_mime_type": "application/json"} if json_mode else None
    ultimo_error = None
    for modelo_actual in (MODELO_PRINCIPAL, MODELO_RESPALDO):
        for i, cliente_actual in enumerate(_CLIENTES):
            try:
                if config:
                    resp = cliente_actual.models.generate_content(
                        model=modelo_actual, contents=contents, config=config
                    )
                else:
                    resp = cliente_actual.models.generate_content(model=modelo_actual, contents=contents)
                if modelo_actual == MODELO_RESPALDO:
                    print(f"⚠️ Usando modelo de respaldo ({MODELO_RESPALDO}) — el principal no tenía cuota libre en ninguna clave.")
                return resp
                ...
                if not es_cuota:
                    raise
                print(f"⚠️ Clave #{i + 1} sin cuota para {modelo_actual}.")
    raise ultimo_error
...
1. Parta 100% de lo que la persona dijo — nombrá algo concreto y específico de
   su relato (una palabra, una situación puntual que haya mencionado), no una
   validación genérica que serviría para cualquiera.
2. {"Usá las señales de rostro y cuerpo SOLO para calibrar el tono y la calidez de tu respuesta (más suave, más directa, más pausada) — regís aplicando la regla ya establecida arriba: nunca las nombres ni las describas de ninguna forma, ni siquiera en lenguaje cotidiano." if hay_multimodal else "Reconozca el tono del relato (posibilidad o limitación) y lo que eso dice de este momento."}
3. Solo si el relato en sí (sus palabras, lo que cuenta) da una base clara y
   específica para hacerlo, podés mencionar si la energía está alta/dinámica
   o si el momento pide una pausa — pero SIEMPRE anclado a algo concreto que
   la persona dijo, nunca como conclusión por defecto ni derivada de una
   señal de rostro/cuerpo. Si no hay base clara en el relato para esa
   lectura, NO la fuerces: seguí directamente con el punto 4. PROHIBIDO usar
   frases tipo "energía baja y de pausa, como si necesitaras un momento para
   tomar aire" salvo que el relato realmente lo respalde de forma específica
   — esta frase en particular ya generó devoluciones desconectadas del
   relato real más de una vez.
...
"""
    ...
    for intento in range(2):
        try:
            response = _generar(prompt, json_mode=True)
...
def continuar_conversacion(
    ...
    # FIX PRIORIDAD 1 (parte 2): el fundamento teórico curado se estaba
    # inyectando solo en las consignas iniciales (ejercicio, bitácora,
    # meditación), pero NUNCA en la charla que sigue después — que es
    # donde pasa la mayor parte de la conversación real. Por eso las
    # devoluciones en el chat podían sentirse genéricas, sin anclaje al
    # marco teórico curado de ese dominio/herramienta puntual.
    contexto_teorico_chat = contexto_para_dominio(dominio, herramienta) if dominio else ""

    prompt = f"""{SYSTEM_PROMPT}

{contexto_teorico_chat}
...
4. Si la persona parece llegar a un cierre natural (dice que está mejor, que quiere terminar,
   que ya tiene claridad), marcalo y validá ese cierre con calidez — pero NUNCA lo redactes como
   una invitación a que se vaya ni sugieras vos terminar la charla (evitá frases tipo "avisame y
   cerramos la sesión" o "cuando quieras podemos ir cerrando"). Si la persona quiere terminar, lo
   va a decir ella misma — tu trabajo acá es solo reconocer que llegó a un buen lugar, no
...
# -----------------------------------------------------------------
# EJERCICIO DEL MOMENTO — GUIADO POR PASOS
# Misma estructura que la meditación (círculo + puntos + voz + música),
# pero con identidad propia: menos pasos, más cortos, estilo ACTIVO y
# concreto (algo para HACER ahora, no para habitar pasivamente como la
# meditación). Se apoya en los ejemplos curados de "practica_guiada".
# -----------------------------------------------------------------
def generar_ejercicio_guiado(
    dominio: str,
    relato_texto: str,
    variante_idx: int,
) -> dict:
    """Genera un ejercicio del momento como lista corta de pasos activos.
    Devuelve {"intro": str, "pasos": [...], "fundamento": str,
    "pregunta_cierre": str}.
    """
    contexto = contexto_para_dominio(dominio, "practica_guiada")

    ESTILO_POR_DOMINIO_EJERCICIO = {
        "Cuerpo": (
            "Estilo: ACCIÓN FÍSICA CONCRETA E INMEDIATA. Los pasos son "
            "instrucciones cortas para HACER algo con el cuerpo ahora mismo "
            "(postura, respiración medida, tensar y soltar). A diferencia de "
            "la meditación, acá no se trata de habitar pasivamente sino de "
            "ejecutar una consigna breve y notar el efecto."
        ),
        "Lenguaje": (
            "Estilo: REFORMULACIÓN ACTIVA PASO A PASO. Los pasos guían a la "
            "persona a identificar una frase/juicio concreto y reescribirlo "
            "en el momento, en voz alta o mentalmente, paso por paso — es un "
            "ejercicio de hacer, no de repetir un mantra como en la meditación."
        ),
        "Emocion": (
            "Estilo: NOMBRAR Y ACCIONAR. Los pasos guían a identificar con "
            "precisión la emoción presente y elegir una micro-acción concreta "
            "vinculada a ella — distinto de la meditación, que busca aceptar "
            "sin accionar."
        ),
    }
    estilo = ESTILO_POR_DOMINIO_EJERCICIO.get(dominio, ESTILO_POR_DOMINIO_EJERCICIO["Emocion"])

    prompt = f"""{SYSTEM_PROMPT}

{contexto}

La persona eligió "Ejercicio del momento" en el dominio "{dominio}".
Su relato: "{relato_texto}"
Esta es la variante número {variante_idx + 1} que recibe en esta combinación
durante la sesión — generá algo distinto de variantes anteriores.

{estilo}

Generá un ejercicio guiado breve, como una secuencia corta de pasos que se
leen en voz alta, con pausas entre paso y paso para que la persona los haga
sin mirar la pantalla. A diferencia de la meditación (8-12 pasos, tono
contemplativo), este ejercicio es MÁS CORTO y más activo.

Reglas:
- Entre 4 y 6 pasos (más corto que una meditación — es "algo breve para
  hacer ahora", no una sesión larga)
- Cada instrucción: máximo 18 palabras
- pausa_seg: 3 a 8 segundos según la acción (acción rápida: 3-4, respiración
  o sostener algo: 6-8)
- El ejercicio tiene que estar conectado al relato concreto de la persona
- Después de los pasos, escribí un "fundamento" (2-3 oraciones) que explique
  el "para qué" de este ejercicio puntual, anclado en el marco teórico
- Terminar con una pregunta de cierre concreta para arrancar la charla
  después — tiene que hacer referencia directa a lo que el ejercicio pidió
  hacer (ej: si pidió notar una sensación, preguntar por esa sensación
  puntual, nunca algo genérico como "¿cómo te sentís?")

Respondé en JSON estricto:
{{
  "intro": "Una oración de introducción breve",
  "pasos": [
    {{"texto": "Instrucción...", "pausa_seg": 4}},
    ...
  ],
  "fundamento": "Para qué sirve este ejercicio puntual...",
  "pregunta_cierre": "Pregunta concreta de seguimiento..."
}}
"""
    ...
    resguardo_pasos = {
        ...
        "pasos": [
            {"texto": "Sentate con los pies bien apoyados en el suelo.", "pausa_seg": 3},
            {"texto": "Inhalá profundo llevando el aire al abdomen.", "pausa_seg": 5},
            {"texto": "Exhalá soltando los hombros.", "pausa_seg": 5},
            {"texto": "Notá el punto de apoyo de tu cuerpo ahora.", "pausa_seg": 5},
        ],
        "fundamento": "Anclar la atención en una sensación física presente ayuda a interrumpir el círculo de pensamientos repetitivos.",
    ...
    for intento in range(2):
        try:
            ...
            bruto = response.text.strip().replace("```json", "").replace("```", "").strip()
            data = json.loads(bruto)
            # FIX: antes solo se validaba que "pasos" fuera una lista, sin
            # chequear su longitud — si Gemini devolvía un ejercicio con
            # 1 solo paso (incumpliendo la consigna de 4 a 6), igual se
            # aceptaba como válido y llegaba así al usuario. Ahora se exige
            # un mínimo de 3 pasos; si no se cumple, se reintenta como con
            # cualquier otro error de formato.
            if "pasos" not in data or not isinstance(data["pasos"], list) or len(data["pasos"]) < 3:
                raise ValueError(f"Pasos insuficientes o formato inválido ({len(data.get('pasos', []))} pasos)")
            return data
        except Exception as e:
            print(f"⚠️ Error ejercicio guiado (intento {intento + 1}): {e}")
