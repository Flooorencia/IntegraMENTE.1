...
# GEMINI_API_KEY_2 es opcional: si está cargada, la app pasa a ella sola
# cuando la primera clave se queda sin cuota diaria (plan gratis: 20/día
...
def _generar(contents):
    """Genera contenido con la clave principal. Si se quedó sin cuota
    diaria (429 / RESOURCE_EXHAUSTED) y hay una clave secundaria cargada,
    pasa a ella automáticamente, sin que el usuario note nada.
    ...
    if client:
        try:
            ...
            if not (es_cuota and client_secundario):
                raise
            print("⚠️ Clave principal de Gemini sin cuota — paso a la clave secundaria.")
    if client_secundario:
        return client_secundario.models.generate_content(model=MODELO, contents=contents)
