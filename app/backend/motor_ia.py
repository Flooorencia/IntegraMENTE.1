...
4. Termine invitando a seguir contando un poco más — con una pregunta abierta,
   concreta y conectada a lo que ya dijo (NO invites todavía a elegir un dominio,
   eso viene después de escuchar un poco más).
...
"""
    ...
    if not client:
        ...
        if herramienta == "bitacora":
            resultado.update(generar_mantra(relato_texto))
        return resultado
    ...
    for intento in range(2):
        try:
            ...
            if herramienta == "bitacora":
                data.update(generar_mantra(relato_texto))
            return data
        except Exception as e:
...
# -----------------------------------------------------------------
# MANTRA PERSONALIZADO — usado en la herramienta "Bitácora" (Lenguaje)
# -----------------------------------------------------------------
def generar_mantra(relato_texto: str) -> dict:
    """
    Genera una frase corta para repetir (mantra) en base a lo que la
    persona escribió, más una explicación breve de cómo puede ayudarla.
    Devuelve {"mantra": str, "explicacion": str}.
    """
    prompt = f"""{SYSTEM_PROMPT}

La persona escribió lo siguiente en su bitácora personal:
"{relato_texto}"

Generá una frase corta (máximo 10 palabras) para que la persona repita
cuando lo necesite — en primera persona, tiempo presente, en lenguaje de
posibilidad (nunca de exigencia). Después, explicá en 1-2 oraciones cómo
esa frase se conecta con lo que escribió y de qué forma repetirla puede
ayudarla.
...
"""
    ...
    resguardo = {
        "mantra": "Voy a mi ritmo, y eso también es avanzar.",
        "explicacion": "Repetirla ayuda a bajar la exigencia del momento y a recordar que el proceso también cuenta.",
    }
    ...
    for intento in range(2):
        try:
            ...
            print(f"⚠️ Error mantra (intento {intento + 1}): {e}")
