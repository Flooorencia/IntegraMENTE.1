...
# FIX CUOTA: antes se usaba el alias "gemini-flash-latest", que Google
# mueve solo hacia el modelo Flash más nuevo disponible. El problema es
# que esa cuota gratis de 20/día es POR MODELO — cuando salió Gemini 3.6
# Flash (21/07/2026) el alias empezó a apuntar ahí sin avisar, y como es
# el modelo más nuevo y más demandado del momento, su cuota gratis se
# agota rápido con tráfico de todo el mundo. Gemini 2.5 Flash es algo más
# lento y algo más caro por token que 3.6, pero tiene su propia cuota de
# 20/día separada y sin esa presión de demanda inicial — más previsible
# para testeo. Fijar la versión exacta (en vez de un alias que Google
# puede mover) evita que la cuota se mueva de abajo sin que nadie lo pida.
...
def _generar(contents, json_mode: bool = False):
    ...
    hablando, es el texto de emergencia. Esto hacía que ejercicios y
    devoluciones se sintieran "menos personalizados" de forma intermitente,
    sin ningún aviso visible de que había fallado. Con esto, esa caída
    debería ser rarísima — el resguardo queda solo para una caída real
    de las tres claves, no para un tropiezo de formato."""
    if not _CLIENTES:
    ...
    for i, cliente_actual in enumerate(_CLIENTES):
        try:
            if config:
                return cliente_actual.models.generate_content(
