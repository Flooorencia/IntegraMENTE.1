...
// =================================================================
// FONDO "BANDERA ONDEANDO" — pantalla-bienvenida / despedida / sesión
// finalizada. Reemplaza al intento anterior con filtro CSS (se veía
// distinto entre celulares y no lograba el efecto de tela real). Acá se
// dibuja el degradé de marca en un <canvas>, fila por fila, desplazando
// cada fila horizontalmente según una onda — el mismo principio que una
// bandera de tela real: cada franja se mueve un poco distinto según la
// altura, y una franja de brillo se desliza por encima simulando el
// pliegue de la tela. Solo corre mientras la pantalla está activa
// (se detiene al salir) para no gastar batería de más.
// =================================================================
let _banderaRAF = null;
let _banderaCanvas = null;
let _banderaCtx = null;
let _banderaTextura = null;
let _banderaAncho = 0;
let _banderaAlto = 0;

function _prepararTexturaBandera(ancho, alto) {
  const off = document.createElement("canvas");
  off.width = ancho;
  off.height = alto;
  const octx = off.getContext("2d");
  const grad = octx.createLinearGradient(0, 0, ancho, alto);
  grad.addColorStop(0, "#000000");
  grad.addColorStop(0.45, "#C21E5D");
  grad.addColorStop(1, "#000000");
  octx.fillStyle = grad;
  octx.fillRect(0, 0, ancho, alto);
  return off;
}

function _dibujarFrameBandera(tMs) {
  if (!_banderaCtx || !_banderaTextura) return;
  const t = tMs / 1000;
  const ctx = _banderaCtx;
  const w = _banderaAncho, h = _banderaAlto;
  ctx.clearRect(0, 0, w, h);
  const STRIP = 3; // alto de cada franja horizontal — más chico = más suave, más costo
  for (let y = 0; y < h; y += STRIP) {
    // Dos ondas superpuestas de distinta frecuencia/velocidad — evita que
    // se vea como un movimiento mecánico repetitivo, más parecido a tela.
    const onda1 = Math.sin(y * 0.018 + t * 1.3);
    const onda2 = Math.sin(y * 0.007 - t * 0.7);
    const desplazamientoX = onda1 * 14 + onda2 * 9;
    ctx.drawImage(_banderaTextura, 0, y, w, STRIP, desplazamientoX, y, w, STRIP);
    // Brillo/sombra de pliegue: una franja clara u oscura sutil según la
    // fase de la onda, para simular cómo la luz cae distinto en cada
    // parte de la tela que ondea.
    const brillo = onda1 * 0.06;
    ctx.fillStyle = brillo >= 0
      ? `rgba(255,255,255,${brillo.toFixed(3)})`
      : `rgba(0,0,0,${(-brillo).toFixed(3)})`;
    ctx.fillRect(0, y, w, STRIP);
  }
  _banderaRAF = requestAnimationFrame(_dibujarFrameBandera);
}

function iniciarFondoBandera(seccionEl) {
  detenerFondoBandera();
  const canvas = seccionEl?.querySelector(".fondo-bandera");
  if (!canvas) return;
  const dpr = Math.min(window.devicePixelRatio || 1, 2);
  const rect = seccionEl.getBoundingClientRect();
  _banderaAncho = Math.max(1, Math.round(rect.width * dpr));
  _banderaAlto = Math.max(1, Math.round(rect.height * dpr));
  canvas.width = _banderaAncho;
  canvas.height = _banderaAlto;
  _banderaCanvas = canvas;
  _banderaCtx = canvas.getContext("2d");
  _banderaTextura = _prepararTexturaBandera(_banderaAncho, _banderaAlto);
  _banderaRAF = requestAnimationFrame(_dibujarFrameBandera);
}

function detenerFondoBandera() {
  if (_banderaRAF) cancelAnimationFrame(_banderaRAF);
  _banderaRAF = null;
  _banderaCanvas = null;
  _banderaCtx = null;
  _banderaTextura = null;
}

const _PANTALLAS_CON_BANDERA = ["pantalla-bienvenida", "pantalla-despedida", "pantalla-sesion-finalizada"];
...
function mostrarPantalla(id, opts = {}) {
  ...
  // El fondo de bandera solo corre en las 3 pantallas que lo usan, y se
  // apaga en cualquier otra — así no gasta batería fuera de esas 3.
  if (_PANTALLAS_CON_BANDERA.includes(id)) {
    iniciarFondoBandera(seccionNueva);
  } else {
    detenerFondoBandera();
  }
...
// Arranque inicial: la pantalla de bienvenida ya está "activa" en el HTML
// antes de que corra este script, así que el fondo de bandera se inicia
// a mano acá (mostrarPantalla todavía no se llamó para esta primera vez).
iniciarFondoBandera(document.getElementById("pantalla-bienvenida"));

// Si el celular rota o cambia de tamaño de ventana, se re-arma el canvas
// con las nuevas medidas para que no quede recortado ni borroso.
window.addEventListener("resize", () => {
  if (_PANTALLAS_CON_BANDERA.includes(pantallaActualId)) {
    iniciarFondoBandera(document.getElementById(pantallaActualId));
  }
});
