...
async function pedirEjercicio() {
  ...
  if (pasosEjercicio[0]) {
    fetchAudioPaso(pasosEjercicio[0].texto, "/tts").then(b64 => { cacheAudioEjercicio[0] = b64; });
  }
...
async function avanzarPasoEjercicio() {
  ...
  if (siguiente && cacheAudioEjercicio[indicePasoEjercicio + 1] === undefined) {
    fetchAudioPaso(siguiente.texto, "/tts").then(b64 => { cacheAudioEjercicio[indicePasoEjercicio + 1] = b64; });
  }

  let audioB64 = cacheAudioEjercicio[indicePasoEjercicio];
  if (audioB64 === undefined) {
    audioB64 = await fetchAudioPaso(paso.texto, "/tts");
...
// endpoint por defecto: "/meditacion/audio-paso" (voz premium, se usa para
// los pasos de Meditación guiada). Los pasos de Ejercicio del momento
// pasan explícitamente "/tts" para usar la voz normal — ElevenLabs queda
// reservado únicamente para Meditación.
async function fetchAudioPaso(texto, endpoint = "/meditacion/audio-paso") {
...
function renderizarMetricas() {
  ...
  if (contEnergia) {
    if (!puntos.length) {
      ...
      const ANCHO = Math.max(360, PAD_IZQ + PAD_DER + fechasVistas.length * 64);
