...
async function finalizarEjercicioYPasarAlChat() {
  ...
  if (textoEl) textoEl.textContent = "Listo, terminaste el ejercicio.";
  ...
  try {
    ...
    // Sin audio disponible: el chat sigue funcionando igual, solo en silencio.
  }
  ...
  // Pausa breve mostrando el cierre antes de pasar a la charla — evita
  // el corte brusco que se sentía antes.
  setTimeout(() => iniciarChatDesdeEjercicio(""), 1600);
}
...
function renderizarMetricas() {
  ...
  if (contEnergia) {
    if (!puntos.length) {
      ...
      const PAD_IZQ = 92, PAD_DER = 24, PAD_TOP = 26, PAD_BOT = 34;
      ...
      const pasoY = (ALTO - PAD_TOP - PAD_BOT) / (ORDEN_ANIMO.length - 1);
      ...
      puntos.forEach(p => {
        ...
        const r = 6 + Math.min(cantidad - 1, 5) * 2.4;
