...
const PANTALLAS_SIN_MENU  = new Set(["pantalla-bienvenida", "pantalla-registro", "pantalla-usuario-creado"]);
...
function mostrarPantalla(id, opts = {}) {
  ...
  if (id !== "pantalla-meditacion") detenerMusicaFondo();
  if (botonAudioActivo) {
...
async function enviarADiagnostico() {
  ...
  try {
    ...
    registrarEvento(`Diagnóstico inicial: ${data.texto}`);
    iniciarChatConversacional(data.texto, "pre_dominio");
...
function configurarRenderEjercicio(variante) {
  document.getElementById("bloque-sol").classList.add("oculto");
  ...
  if (estado.herramientaActual === "practica_guiada") {
    document.getElementById("bloque-sol").classList.remove("oculto");
    reiniciarSol();
...
// -----------------------------------------------------------------
// SOL DORADO — "presioná y sostené", reemplaza el círculo de
// respiración en Ejercicio del momento (ese quedó solo para
// Meditación guiada de Cuerpo).
// -----------------------------------------------------------------
const SOL_DURACION_MS = 2200;
const SOL_CIRCUNFERENCIA = 2 * Math.PI * 52;
let solHoldStart = null;
let solHoldRAF = null;

function reiniciarSol() {
  cancelAnimationFrame(solHoldRAF);
  solHoldStart = null;
  const circ = document.getElementById("sol-anillo-progreso");
  const texto = document.getElementById("sol-texto");
  const boton = document.getElementById("btn-sol");
  if (circ) {
    circ.style.transition = "none";
    circ.style.strokeDasharray = `${SOL_CIRCUNFERENCIA}`;
    circ.style.strokeDashoffset = `${SOL_CIRCUNFERENCIA}`;
  }
  if (texto) texto.textContent = "Presioná y sostené";
  boton?.classList.remove("sol-completo");
}

function iniciarSostenerSol(e) {
  e.preventDefault();
  const circ = document.getElementById("sol-anillo-progreso");
  if (!circ) return;
  solHoldStart = performance.now();
  circ.style.transition = "none";
  cancelAnimationFrame(solHoldRAF);
  const paso = () => {
    const t = Math.min((performance.now() - solHoldStart) / SOL_DURACION_MS, 1);
    circ.style.strokeDashoffset = `${SOL_CIRCUNFERENCIA * (1 - t)}`;
    if (t >= 1) { completarSol(); return; }
    solHoldRAF = requestAnimationFrame(paso);
  };
  solHoldRAF = requestAnimationFrame(paso);
}

function soltarSol() {
  if (solHoldStart === null) return;
  cancelAnimationFrame(solHoldRAF);
  solHoldStart = null;
  const circ = document.getElementById("sol-anillo-progreso");
  if (circ) {
    circ.style.transition = "stroke-dashoffset 0.4s ease";
    circ.style.strokeDashoffset = `${SOL_CIRCUNFERENCIA}`;
  }
}

function completarSol() {
  const texto = document.getElementById("sol-texto");
  if (texto) texto.textContent = "✨ Completado";
  document.getElementById("btn-sol")?.classList.add("sol-completo");
}

const btnSol = document.getElementById("btn-sol");
btnSol?.addEventListener("pointerdown", iniciarSostenerSol);
btnSol?.addEventListener("pointerup", soltarSol);
btnSol?.addEventListener("pointerleave", soltarSol);
btnSol?.addEventListener("pointercancel", soltarSol);
...
document.getElementById("btn-continuar-ejercicio").addEventListener("click", () => {
  ...
  const tarjetaMantra = document.getElementById("tarjeta-mantra");
  if (data.mantra) {
    document.getElementById("texto-mantra").textContent = data.mantra;
    document.getElementById("texto-mantra-explicacion").textContent = data.mantra_explicacion || "";
    document.getElementById("btn-reproducir-mantra").onclick =
      (e) => reproducirAudioBase64(data.audio_mantra_base64, e.currentTarget);
    tarjetaMantra.classList.remove("oculto");
  } else {
    tarjetaMantra.classList.add("oculto");
  }
...
document.getElementById("btn-continuar-a-ruteo").addEventListener("click", () => {
  const pregunta = estado.ejercicioActualDetalle?.pregunta_seguimiento;
  iniciarChatConversacional(pregunta, "post_ejercicio");
});
...
function iniciarMusicaFondo() {
  ...
  musicaFondoEl.play().catch(() => { /* si el archivo no existe todavía, no pasa nada */ });
}
...
document.getElementById("btn-saltear-meditacion")?.addEventListener("click", () => {
  ...
  iniciarChatConversacional(preguntaCierreMeditacion, "post_ejercicio");
});
...
let chatModoActual = "post_ejercicio"; // "pre_dominio" | "post_ejercicio"

function iniciarChatConversacional(preguntaInicial, modo = "post_ejercicio") {
  chatModoActual = modo;
  ...
  if (btnFinalizar) {
    // El botón aparece recién después de un intercambio real — así nadie
    // sale sin haber tenido, al menos, una charla mínima.
...
async function enviarMensajeChat() {
  ...
  try {
    ...
    // Primer intercambio real completo: ahora sí mostramos "Continuar".
    document.getElementById("btn-chat-finalizar")?.classList.remove("oculto");
...
document.getElementById("btn-chat-finalizar")?.addEventListener("click", async () => {
  if (chatModoActual === "pre_dominio") {
    await avanzarAExplicacionProfunda();
  } else {
    mostrarPantalla("pantalla-ruteo");
  }
});

async function avanzarAExplicacionProfunda() {
  mostrarPantalla("pantalla-analisis");
  ...
  let data;
  try {
    const resp = await fetch(`${API_BASE}/explicacion`, { method: "POST", body: form });
    data = await resp.json();
  } catch (e) {
    data = {
      texto: "Gracias por contarme todo esto. Elegí un dominio para seguir trabajando: Cuerpo, Lenguaje o Emoción.",
      audio_base64: "",
    };
  }

  document.getElementById("texto-diagnostico").textContent = data.texto;
  registrarEvento(`Explicación: ${data.texto}`);
  document.getElementById("btn-reproducir-diagnostico").onclick =
