...
function configurarRenderEjercicio() {
  document.getElementById("bloque-presencia").classList.remove("oculto");
...
// -----------------------------------------------------------------
// DESLIZADOR DE PRESENCIA — antes/después de "Ejercicio del momento"
// -----------------------------------------------------------------
function reiniciarPresencia() {
  const antes = document.getElementById("slider-presencia-antes");
  const despues = document.getElementById("slider-presencia-despues");
  if (antes) { antes.value = 3; document.getElementById("valor-presencia-antes").textContent = "3"; }
  if (despues) { despues.value = 3; document.getElementById("valor-presencia-despues").textContent = "3"; }
}

document.getElementById("slider-presencia-antes")?.addEventListener("input", (e) => {
  document.getElementById("valor-presencia-antes").textContent = e.currentTarget.value;
});
document.getElementById("slider-presencia-despues")?.addEventListener("input", (e) => {
  document.getElementById("valor-presencia-despues").textContent = e.currentTarget.value;
});

document.getElementById("btn-continuar-ejercicio").addEventListener("click", () => {
  const antes = document.getElementById("slider-presencia-antes")?.value;
  const despues = document.getElementById("slider-presencia-despues")?.value;
  if (antes && despues) {
    registrarEvento(`Presencia antes/después del ejercicio: ${antes} → ${despues}.`);
  }
  ...
  if (data.audio_fundamento_base64 && !audioSilenciado) {
    reproducirAudioBase64(data.audio_fundamento_base64, document.getElementById("btn-reproducir-fundamento"));
  }
});
...
document.getElementById("btn-enviar-bitacora")?.addEventListener("click", async () => {
  ...
  if (!audioSilenciado) {
    if (data.audio_insight_base64) {
      reproducirAudioAutomatico(data.audio_insight_base64, () => {
        if (data.audio_frase_base64) reproducirAudioAutomatico(data.audio_frase_base64, () => {});
      });
    } else if (data.audio_frase_base64) {
