...
document.getElementById("btn-grabar-chat")?.addEventListener("click", async function () {
  ...
  if (!grabandoChat) {
    try {
      ...
      // FIX: estado de grabación explícito en texto, no solo color —
      // arranca en "Grabando..." y a los 800ms pasa a indicar la acción
      // siguiente ("Parar grabación"), así la persona nunca tiene que adivinar.
      if (estadoEl) {
        estadoEl.textContent = "Grabando...";
        estadoEl.classList.remove("oculto");
        setTimeout(() => { if (grabandoChat && estadoEl) estadoEl.textContent = "Parar grabación"; }, 800);
      }
