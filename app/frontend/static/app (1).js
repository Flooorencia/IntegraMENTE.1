...
async function pedirEjercicio() {
  ...
  try {
    ...
    const resp = await fetch(`${API_BASE}/ejercicio-guiado`, { method: "POST", body: form, signal: controller.signal });
    clearTimeout(timeoutId);
    data = await resp.json();
    if (!data.pasos || !Array.isArray(data.pasos) || data.pasos.length === 0) {
      throw new Error("Respuesta sin pasos válidos");
    }
  } catch (e) {
    // FIX: este resguardo antes tenía un solo paso hardcodeado, así que
    // si el pedido al backend fallaba o se cortaba por timeout (Gemini
    // lento, cold start de Render, etc.), el ejercicio se mostraba con
    // una única instrucción en vez de una secuencia completa. Ahora usa
    // los mismos 4 pasos de emergencia que ya existían del lado del
    // backend (motor_ia.py → generar_ejercicio_guiado), para que incluso
    // en el peor caso la persona reciba un ejercicio completo, no roto.
    console.warn("⚠️ Ejercicio guiado: usando resguardo local por falla de red/timeout.", e);
    data = {
