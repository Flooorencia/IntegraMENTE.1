...
document.getElementById("menu-item-silenciar")?.addEventListener("click", (e) => {
  ...
  e.currentTarget.textContent = audioSilenciado ? "🔇 Activar audio" : "🔊 Silenciar audio";
...
async function generarReporteAutomatico() {
  ...
  try {
    ...
    if (estadoTexto) estadoTexto.textContent = "Tu reporte quedó guardado — lo encontrás en el menú, en \"Reportes\".";
  } catch (e) {
    console.warn("Error generando reporte:", e);
    if (estadoTexto) estadoTexto.textContent = "No pudimos guardar el reporte esta vez, pero tu sesión ya quedó registrada.";
  }
}
